...
export default function generateLayout (code, items = [], inFreeLayout = false) {
    // return 'page-content'
    const commonTemplateTag = transformTemplates(code.isMiniProgram, 'template')
    const commonDivTag = transformTemplates(code.isMiniProgram, 'div')
    ...
    for (let i = 0; i < len; i++) {
        ...
        if (templateDirective) templateCode += `\n<${commonTemplateTag} ${templateDirective}>`
        // 或者对布局设置的居中、居左、居右样式
        ...
        if (item.type === 'render-grid') {
            ...
            templateCode += `
                <${commonDivTag}${alignStr}class="bk-layout-row-${code.uniqueKey} ${item.componentId}" ${vueDirective} ${propDirective}>
                    ${item.renderSlots && item.renderSlots.default && item.renderSlots.default.map(col => {
                        ...
                        let rowCode = ''
                        if (rowTempDirs.join(' ')) rowCode += `\n<${commonTemplateTag} ${rowTempDirs.join(' ')}>`
                        rowCode += `<${commonDivTag} class="bk-layout-col-${code.uniqueKey} ${col.componentId}"${colAlignStr} ${rowVueDirs.join(' ')} ${rowPropDirs.join(' ')}>
        ...
        // 自由布局
            ...
            templateCode += `
                <${commonDivTag} class="bk-free-layout-${code.uniqueKey} ${item.componentId}" ${vueDirective} ${propDirective}>
                    <${commonDivTag} class="bk-free-layout-item-inner-${code.uniqueKey}">
                        ${generateLayout(code, item.renderSlots.default || [], true)}
                    </${commonDivTag}>
                </${commonDivTag}>
            `
            /* eslint-enable no-unused-vars, indent */ // #miniProgram-to-do H5 容器不支持
