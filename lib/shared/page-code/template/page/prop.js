...
function getPropsStr (code, type, compId, props, dirProps, slots) {
    ...
    for (const i in props) {
        if ((dirProps.find((directive) => (directive.prop === i)) && !['remote', 'data-source', 'table-data-source'].includes(props[i].valueType))
            || (['bk-checkbox-group', 'bk-radio-group'].includes(type) && ['value', 'modelValue'].includes(i))) continue
        ...
        if (i !== 'slots' && i !== 'class') {
            ...
            if (format !== 'value' && modifiers?.length > 0) {
                propName = `${i}.${modifiers.join('.')}` // #miniProgram-to-do 检索移动端是否有sync属性
            }
            // 设置了directive指令
            if (directive) {
            ...
            const curPropStr = code.isMiniProgram
                ? `${propName}="${val === undefined || directive ? propVar : `{{${propVar}}}`}" `
                : `${val === undefined || directive ? '' : ':'}${propName}="${propVar}" `
            ...
            // h5容器动画属性的特殊处理, 将数字转为字符串  3 => 3s
                ...
                if (val !== undefined) {
                    ...
                    propsStr += code.isMiniProgram
                        ? `${propName}="${typeof val === 'string' ? v : `{{${v}}}`}" `
                        : `${typeof val === 'string' ? '' : ':'}${propName}="${v}" `
                }
            }
        }
    }
    ...
    if (type === 'bk-checkbox-group' && !hasVModel) {
        ...
        const checkedValue = (slots.default.code || []).filter(c => c.checked === true).map(c => c.value) || propVal
    ...
    if (type.startsWith('el-') || type.startsWith('van')) {
        if (!hasVModel && modelComId !== '') {
            ...
            propsStr += code.isMiniProgram ? `model:value="{{${props['value'].format === 'variable' ? props['value'].code : modelComId}}}"` : `v-model="${props['value'].format === 'variable' ? props['value'].code : modelComId}"`
        }
    }
