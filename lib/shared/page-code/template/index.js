...
export default function (code) {
    const sectionTag = transformTemplates(code.isMiniProgram, 'section')
    const templateTag = transformTemplates(code.isMiniProgram, 'template')
    let pageCode = ''
    if (code.isEmpty) { // 空白页面、404时
        ...
        pageCode = `\n<${sectionTag} class="bk-layout-custom-component-wrapper container-${code.uniqueKey}">\n${generateNocodePageCode(code)}\n</${sectionTag}>\n`
    } else { // 普通自定义页面
