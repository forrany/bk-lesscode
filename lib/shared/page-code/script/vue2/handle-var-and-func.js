import { replaceFuncKeyword, replaceFuncParam, getRemoteFunctionInfo, generateValidate } from '../../../function/helper'
...
export default function handleUsedVarAndFunc (code) {
    while (code.unhandledFunc.length > 0 || code.unhandledVariables.length > 0) {
        // 处理函数
        console.log('unhandleFunc', code.unhandledFunc, code.unhandledVariables, code.funcGroups)
            ...
            if (
                ...
                if (variable.valueType !== VARIABLE_TYPE.COMPUTED.VAL) {
                    ...
                    if ([VARIABLE_TYPE.ARRAY.VAL, VARIABLE_TYPE.OBJECT.VAL].includes(variable.valueType)) {
                        ['all', 'prod', 'stag', 'preview'].forEach((key) => {
                ...
                // 处理计算变量
                if (variable.valueType === VARIABLE_TYPE.COMPUTED.VAL) {
                    variable.defaultValue.all = processFuncBody(variable.defaultValue.all)
...
function getCompleteFuncByCode (code, methodCode) {
    ...
    if (returnMethod.funcType === 1) {
        ...
        const data = `{
            url: \`${processFuncUrl(code, funcApiUrl)}\`,
        ...
        const {
            validateString,
            codes: funcCodes
        } = generateValidate(apiDataString, apiHeaderString, returnMethod)
        funcCodes.forEach((funcCode) => {
            handleVarInFunc(code, null, funcCode)
        })
        returnMethod.code = addFuncStr(`${validateString} return this.$store.dispatch('getApiData', ${data}).then((${remoteParams}) => { ${funcBody} }).catch((err) => { console.error(err) })`)
    } else {
 ...
 * @returns { String }
 */
function handleVarInFunc (code, dirKey, funcCode) {
    if (funcCode) {
        const [curFunc] = getMethodByCode(funcCode, code.funcGroups)
        if (curFunc.id) {
            code.addUsedFunc(funcCode)
        } else {
            code.codeErrMessage = `${sharedI18n().t('函数')}【${funcCode}】${sharedI18n().t('不存在，函数执行可能存在异常，请修改后再试')}`
        }
        return `this.${curFunc.funcName}`
    }
    if (dirKey) {
        const variable = code.variableList.find((variable) => (variable.variableCode === dirKey))
        if (variable) {
            code.addUsedVariable(variable)
        } else {
            code.codeErrMessage = `${sharedI18n().t('变量')}【${dirKey}】${sharedI18n().t('不存在，函数执行可能存在异常，请修改后再试')}`
        }
        return `this.${dirKey}`
    }
}
...
function processFuncBody (code, bodyCode) {
    ...
    return replaceFuncKeyword(encodeCode, (all, first, second, dirKey, funcStr, funcCode) => {
        return handleVarInFunc(code, dirKey, funcCode) || all
    })
}
...
function processFuncUrl (code, str) {
    return replaceFuncParam(str || '', (variableCode) => {
        return `\$\{${handleVarInFunc(code, variableCode)}\}`
    })
}
