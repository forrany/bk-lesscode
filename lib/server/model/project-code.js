...
const projectCode = {
            ...
            try {
                ...
                for (const layout of layoutIns) {
                    ...
                    const childRoute = routeList.map((route) => {
                        ...
                        const meta = `meta: { pageName: '${route.pageName}', platform: '${pagePlatform}' }`
                ...
                await this.generateDataSource(dataTables, dataTableModifyRecords, targetPath, flowList)
                await this.generateFileByReplace(
                    path.join(targetPath, 'lib/server/service/form.js'),
                    path.join(targetPath, 'lib/server/service/form.js'),
                    (content) => {
                        const user = RequestContext.getCurrentUser()
                        return content.replace(/\$\{apiName\}/, `lesscode-${projectInfo.projectCode}${projectId}`).replace(/\$\{user\}/, user.username)
                    }
                )
                await this.generateFileByReplace(
    ...
    async generateDataSource (dataTables = [], dataTableModifyRecords = [], targetPath, flowList) {
        ...
        await this.generateFileByReplace(appPath, appPath, (content) => {
            ...
            return content
                ...
                .replace(
                    /\$\{gwImport\}/,
                    flowList.length ? 'const { grantApiGWPermissionForItsm } = require(\'./service/form\')' : ''
                )
                .replace(
                    /\$\{grantApiGWPermissionForItsm\}/,
                    flowList.length ? 'await grantApiGWPermissionForItsm()' : ''
                )
        })
    ...
    getNocodePayload (route = {}, codeType) {
        ...
        if (route.nocodeType) {
            if (['FORM', 'FLOW'].includes(route.nocodeType)) {
                Object.assign(nocodePayload, { formId: route.formId, tableName: route.tableName, fields: JSON.parse(route.formContent || '[]'), serviceId: route.flowServiceId, versionId: versionMap[route.flowServiceId] || 0 })
