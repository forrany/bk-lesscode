...
const projectCode = {
    ...
    async previewCode (projectId, versionId, platform = 'PC', pageId = '') {
        // 获取预览相关的数据
        const [
            allRouteList,
            allPageRouteList,
            ...
            LCDataService.findOne(TABLE_FILE_NAME.PROJECT, { id: projectId })
        ])
        ...
        for (const layout of layoutIns) {
            ...
            await Promise.all(routeList.map(async route => {
                ...
                if (pageDetail.codeErrMessage) {
                    ...
                    route.content = ''
                    route.meta = {
                        message: global.i18n.t('页面【{{0}}】里面，{{1}}。请修复后刷新页面再试', [route.pageCode, pageDetail.codeErrMessage])
                    }
                } else {
        ...
        return { routeGroup, storeData, pageRouteList, framework }
    },
    ...
    async generateCode (projectId, versionId, pathName) {
        return new Promise(async (resolve, reject) => {
            try {
                ...
                // 计算资源路径
                const framework = projectInfo.framework === 'vue3' ? 'vue3' : 'vue2'
                const staticUrl = STATIC_URL + framework + '/'
                const sourcePath = staticUrl + 'project-init-code'
                ...
                // 获取生成view文件所需的数据
                let projectVariables = allVarableList.filter(variable => variable.effectiveRange === 0)
                projectVariables = unique(projectVariables, 'variableCode')
                let usedMethodList = []
                const layoutIns = Object.values(routeGroup)
                const uniqStr = uuid()
                ...
                for (const layout of layoutIns) {
                    ...
                    if (layout.path !== '/') childRoute.push(`{ path: \'${allPath}\', component: BkNotFound, meta: { pageName: \'404\' } }`)
                ...
                const routeSourcePath = path.join(staticUrl, 'router-template.js')
                ...
                if (hasMobilePage) {
                    if (framework === 'vue3') {
                        mainFileContent = mainFileContent.replace(/\$\{importVantLib\}/, 'import Vant from \'vant\'\nimport \'vant/lib/index.css\'\nimport widgetVanPicker from \'@/components/patch/widget-van-picker.vue\'\nimport bkLuckyCanvas from \'@/components/patch/bk-lucky-canvas.vue\'')
                        mainFileContent = mainFileContent.replace(/\$\{useVantLib\}/, '.use(Vant)')
                        mainFileContent = mainFileContent.replace(/\$\{useVantWidget\}/, 'global.mainComponent.component(\'widget-van-picker\', widgetVanPicker)\n\t\tglobal.mainComponent.component(\'bk-lucky-canvas\', bkLuckyCanvas)')
                        mainFileContent = mainFileContent.replace(/\$\{remJs\}/, remJS)
                        await this.writePackageJSON(
                            path.join(targetPath, 'package.json'),
                            [
                                { name: 'vant', version: '^4.3.1' },
                    ...
                    mainFileContent = mainFileContent.replace(/\$\{useVantLib\}/, '')
                    mainFileContent = mainFileContent.replace(/\$\{useVantWidget\}/, '')
                ...
                await this.generateDataSource(dataTables, dataTableModifyRecords, targetPath, isGenerateApigw, STATIC_URL)
    ...
    async generateDataSource (dataTables = [], dataTableModifyRecords = [], targetPath = '', isGenerateApigw = false, staticUrl) {
        ...
        if (hasDataTable) {
            ...
            await this.generateFileByReplace(
                path.join(staticUrl, 'data-base-template.js'),
            ...
            for (const dataTable of dataTables) {
                ...
                await this.generateFileByReplace(
                    path.join(staticUrl, 'entity-template.js'),
        ...
        if (dataTableModifyRecords.length) {
            ...
            for (const record of dataTableModifyRecords) {
                ...
                await this.generateFileByReplace(
                    path.join(staticUrl, 'data-migration-template.js'),
    ...
    async generateFileByReplace (sourcePath, targetPath, replaceCallBack = str => str) {
        ...
        const [message, fileStr] = await VueCodeModel.formatJsByEslint(newFileContent, sourcePath)
    ...
    async writeMethodsMixin (methodsMixinPath, usedMethodList, pathName) {
        ...
        usedMethodList.forEach(({ id, funcStr }) => {
            ...
            if (!curStr) {
                ...
                const hasUnknowCode = /lesscode((\[\'\$\{prop:([^}]+)\}\'\])|(\[\'\$\{func:([^}]+)\}\'\]))/.test(curStr)
                if (hasUnknowCode) {
                    methodMap[id] = funcStr
                }
            }
    ...
    writeViewCode ({ currentFilePath, pathName, pageCode, targetData, funcGroups, lifeCycle, projectId, pageId, deletePageCodes, layoutContent, isGenerateNav, layoutType, variableData, styleSetting, apiList, platform = 'PC', nocodeType = '', nocodePayload, framework }) {
        return new Promise(async (resolve, reject) => {
            fse.ensureFile(currentFilePath).then(async () => {
                ...
                if (pageDetail.codeErrMessage && !pathName) {
                    throw new Error(global.i18n.t('页面【{{0}}】里面，{{1}}', [pageCode, pageDetail.codeErrMessage]))
                }
